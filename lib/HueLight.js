...
HueLight.prototype.heartbeat = function(obj) {
  ...
  if (this.state.on !== old.state.on) {
    logger.logCharacteristicUpdateForDevice(this.name,"on",this.state.on);
    this.log.debug("%s: %s power changed from %s to %s", this.name, this.type,
    		   old.state.on, this.state.on);
  }
  if (this.hk.on !== old.hk.on) {
    logger.logCharacteristicUpdateForDevice(this.name,"on",this.hk.on);
  ...
  if (this.state.bri !== undefined && this.state.bri !== old.state.bri) {
    logger.logCharacteristicUpdateForDevice(this.name,"brightness",this.hk.bri);
  ...
  if (this.state.ct !== undefined && this.state.ct !== old.state.ct) {
    if (this.state.colormode === "color temperature") {
  ...
  if (this.hk.ct !== old.hk.ct) {
    logger.logCharacteristicUpdateForDevice(this.name,"color temperature",this.hk.ct);
  ...
  if (this.hk.hue !== old.hk.hue) {
    logger.logCharacteristicUpdateForDevice(this.name,"hue",this.hk.hue);
    this.log.info("%s: set homekit hue from %s˚ to %s˚",
		  this.name, old.hk.hue, this.hk.hue);
    this.service.setCharacteristic(Characteristic.Hue, this.hk.hue);
  }
  if (this.hk.sat !== old.hk.sat) {
    logger.logCharacteristicUpdateForDevice(this.name,"saturation",this.hk.sat);
  ...
  if (this.hk.fault !== old.hk.fault) {
    logger.logCharacteristicUpdateForDevice(this.name,"fault",this.hk.fault);
